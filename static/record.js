...
const dynamicStyleDiv = document.getElementById('dynamic-style');
const setActivePerson = (person_id) => {
  dynamicStyleDiv.innerHTML = `<style>.person_${person_id}{background-color:#cce0ff}</style>`;
};
let current = null;
...
const renderRecord = (() => {
  ...
  return async (init = false) => {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      const start = i === 0 || record.person_id !== records[i - 1].person_id || getDate(record) !== getDate(records[i - 1]);
      const end = i === records.length - 1 || record.person_id !== records[i + 1].person_id || getDate(record) !== getDate(records[i + 1]);
      ...
      const date = start ? `<div class="time">${person.name} ${getDate(record)}</div>` : '';
      let text = null;
      if (record.type === 0) {
        text = replaceLarkEmoji(record.content);
      ...
      const right = `<div class="message-right">${date}${content}</div>`;
